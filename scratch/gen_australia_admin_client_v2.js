...
export function AustraliaDetailsClient({ application, signedPhotoUrls }: Props) {
  ...
  const handleStatusChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    try {
      await updateApplicationStatus(application.id, newStatus, 'australia' as any)
    } catch {
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      await updateApplicationNotes(application.id, notes, 'australia' as any)
    } catch {
  ...
  const handleDelete = async () => {
    if (confirm('¿Estás seguro de eliminar esta solicitud? Esta acción no se puede deshacer.')) {
      setIsDeleting(true)
      try {
        await deleteApplication(application.id, 'australia' as any)
        router.push('/admin')
      } catch {
        ...
        {activeTab === 'datos' ? (
          <>
            ...
            <SectionCard title="Documentos Subidos">
              ...
              <DocumentLink url={signedPhotoUrls.doc_passport_old} label="Pasaporte Antiguo" />
              <DocumentLink url={signedPhotoUrls.doc_national_id} label="Documento Nacional" />
              ...
              <DocumentLink url={signedPhotoUrls.host_doc_sponsor_letter} label="Carta Sponsor" />
            </SectionCard>
          </>
        ) : (
...
console.log('AustraliaDetailsClient.tsx updated');
