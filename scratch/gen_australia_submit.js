...
const submitPath = path.join(__dirname, '../app/aplicar/turismo-australia/_actions/submit-application.ts');
...
async function executeSubmit(formData: any) {
  ...
  try {
    const { error: dbError } = await supabaseAdmin
      ...
      .insert({
        ...
        authorised_recipient: deepNullify(step4.authorised_recipient),
        correspondence_email: nullify(step4.correspondence_email),

        // Step 5
        travelling_with_others: toBool(step5.travelling_with_others),
        travelling_companions: deepNullify(step5.travelling_companions || []),
        ...
        contacts_in_australia: deepNullify(step7.contacts_in_australia || []),
        ...
        supporter: deepNullify(step9.supporter),
        ...
        // Step 14 (Documents)
        doc_passport_current: nullify(step14.doc_passport_current),
        doc_passport_old: nullify(step14.doc_passport_old),
        doc_passport_stamp_pages: nullify(step14.doc_passport_stamp_pages),
        doc_national_id: nullify(step14.doc_national_id),
        doc_usa_visa: nullify(step14.doc_usa_visa),
        doc_civil_registry: nullify(step14.doc_civil_registry),
        doc_children_civil_registries: deepNullify(step14.doc_children_civil_registries || []),
        doc_residence_letter: nullify(step14.doc_residence_letter),
        doc_property_certificates: deepNullify(step14.doc_property_certificates || []),
        doc_vehicle_ownership: nullify(step14.doc_vehicle_ownership),
        doc_bank_statements: deepNullify(step14.doc_bank_statements || []),
        doc_pension_proof: nullify(step14.doc_pension_proof),
        doc_itinerary: nullify(step14.doc_itinerary),
        host_doc_passport_id: nullify(step14.host_doc_passport_id),
        host_doc_australian_visa: nullify(step14.host_doc_australian_visa),
        host_doc_proof_of_address: nullify(step14.host_doc_proof_of_address),
        host_doc_relationship_evidence: nullify(step14.host_doc_relationship_evidence),
        host_doc_payslips: deepNullify(step14.host_doc_payslips || []),
        host_doc_employment_letter: nullify(step14.host_doc_employment_letter),
        host_doc_bank_statements: deepNullify(step14.host_doc_bank_statements || []),
        host_doc_savings: nullify(step14.host_doc_savings),
        host_doc_tenancy_agreement: nullify(step14.host_doc_tenancy_agreement),
        host_doc_sponsor_letter: nullify(step14.host_doc_sponsor_letter),
        host_doc_activity_plan: nullify(step14.host_doc_activity_plan),
      })
  ...
  try {
    const adminHtml = \`
      <div style="font-family: sans-serif; background-color: #0A0A0A; color: #FAFAF7; padding: 40px 20px;">
        <div style="background-color: #1A1A1A; border: 1px solid #333; border-radius: 12px; padding: 30px; max-width: 600px; margin: 0 auto;">
          ...
          <p><strong>Aplicante:</strong> \${step2.given_names || ''} \${step2.family_name}</p>
    ...
    await resend.emails.send({
      from: FROM_EMAIL,
      to: ADMIN_EMAIL,
      subject: \`🇦🇺 Australia / Visa Visitante — \${step2.family_name}\`,
      html: adminHtml,
    })
  ...
  // 3. Send client email (simple text/html for now, since PDF is handled via background or separately)
  try {
    const clientHtml = \`
      <div style="font-family: sans-serif; padding: 40px 20px;">
        <h2>¡Recibimos tu solicitud para Australia!</h2>
        <p>Hola \${step2.given_names},</p>
        <p>Hemos recibido tu aplicación para la Visa de Visitante (Subclass 600) de Australia.</p>
        <p>Nuestro equipo revisará la información y te contactaremos a la brevedad.</p>
        <br/>
        <p>Saludos,</p>
        <p><strong>LATAM VISA</strong></p>
      </div>
    \`
    await resend.emails.send({
      from: FROM_EMAIL,
...
fs.writeFileSync(submitPath, content);
console.log('submit-application.ts updated');
