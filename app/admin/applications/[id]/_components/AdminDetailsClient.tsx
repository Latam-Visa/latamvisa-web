...
import { FileText, Save, Trash2, Loader2, X, ZoomIn } from 'lucide-react'
...
function PhotoThumb({ url, label, onClick }: { url?: string; label: string; onClick: () => void }) {
  if (!url) {
    return (
      <div className="flex items-center gap-2 text-sm text-[#A3A3A3] mt-3">
        <span>📷 {label}: <span className="italic">Sin foto</span></span>
      </div>
    )
  }
  return (
    <div className="mt-3">
      ...
      <button
        type="button"
        onClick={onClick}
        className="relative group w-[120px] h-[160px] rounded-lg overflow-hidden border border-[#E5E5E5] bg-[#F5F5F0] block"
      >
  ...
  return (
    <>
      ...
      {modalPhotoUrl && (
        ...
        >
          <div
            className="relative bg-white rounded-xl overflow-hidden max-w-2xl max-h-[90vh]"
            onClick={e => e.stopPropagation()}
          >
            <button
              onClick={() => setModalPhotoUrl(null)}
              className="absolute top-3 right-3 z-10 bg-white rounded-full p-1 shadow-md hover:bg-[#F5F5F0]"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <SectionCard title="Paso 3 — Pasaporte">
            ...
            <PhotoThumb
              url={signedPhotoUrls.passport}
              label="Foto del pasaporte"
              onClick={() => signedPhotoUrls.passport && setModalPhotoUrl(signedPhotoUrls.passport)}
            />
          </SectionCard>
          ...
          <SectionCard title="Paso 5 — Historial de Visas USA">
            ...
            <PhotoThumb
              url={signedPhotoUrls.previousVisa}
              label="Foto de visa anterior"
              onClick={() => signedPhotoUrls.previousVisa && setModalPhotoUrl(signedPhotoUrls.previousVisa)}
            />
          </SectionCard>
          ...
          <SectionCard title="Paso 8 — Información Adicional">
            ...
            <Row label="¿Antecedentes penales?" value={bool(a.criminalRecord)} />
            <Row label="¿Condiciones médicas?" value={bool(a.medicalConditions)} />
            <Row label="¿Historial deportación?" value={bool(a.deportationHistory)} />
            <PhotoThumb
              url={signedPhotoUrls.visaPhoto}
              label="Foto tipo visa"
              onClick={() => signedPhotoUrls.visaPhoto && setModalPhotoUrl(signedPhotoUrls.visaPhoto)}
            />
          </SectionCard>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[#E5E5E5] p-6 space-y-4">
            <div className="text-xs text-[#525252] space-y-1 border-b border-[#E5E5E5] pb-4">
              ...
              <p><strong>PDF:</strong> {application.pdf_url ? '✅ Generado' : '⏳ Pendiente'}</p>
              <p><strong>Passport foto:</strong> {application.passport_photo_url ? '✅' : '❌'}</p>
              <p><strong>Visa foto:</strong> {application.visa_photo_url ? '✅' : '❌'}</p>
              <p><strong>Visa anterior foto:</strong> {application.previous_visa_photo_url ? '✅' : 'N/A'}</p>
            </div>
