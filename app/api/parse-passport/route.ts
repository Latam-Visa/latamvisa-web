...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2. If PDF: convert first page to JPEG using sharp
    // Note: sharp doesn't natively parse PDFs unless compiled with poppler.
    // If it fails, we catch the error and return it.
    if (file.type === 'application/pdf') {
      try {
        const jpegBuffer = await sharp(buffer, { density: 300 }).jpeg().toBuffer()
      ...
      if (file.type === 'image/jpeg' || file.type === 'image/png' || file.type === 'image/webp' || file.type === 'image/gif') {
        ...
        // convert to jpeg using sharp if it's heic or something else
