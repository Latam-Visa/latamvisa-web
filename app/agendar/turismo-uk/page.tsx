...
function StripeEmbeddedCheckout({ conTraduccion }: { conTraduccion: boolean }) {
  const fetchClientSecret = useCallback(() => {
    return fetch('/api/checkout/turismo-uk', {
      method: 'POST',
...
export default function TurismoUkCheckoutPage() {
  const [conTraduccion, setConTraduccion] = useState(false)

  return (
    <>
      <title>Standard Visitor Visa UK | Checkout Seguro</title>
      <meta name="description" content="Finaliza el pago de tu asesoría de visa de turismo al Reino Unido." />
      ...
      <main className="min-h-screen lg:min-h-0 lg:h-screen lg:overflow-hidden bg-white text-[#111111] flex flex-col-reverse lg:flex-row w-full font-funnel selection:bg-[#111111] selection:text-[#C8FF00]">
        ...
        <section className="w-full lg:w-[45%] lg:h-screen lg:overflow-y-auto text-[#111111] border-b lg:border-b-0 lg:border-l border-[#C8FF00]/40 px-6 py-12 sm:px-10 lg:px-16 lg:py-16 relative bg-white/40 bg-gradient-to-br from-[#012169]/20 to-[#C8102E]/20 backdrop-blur-2xl">
              ...
              <ul className="space-y-3">
                {[
                  ...
                  <li key={idx} className="flex items-center gap-4 group">
                    <span className="shrink-0 w-1.5 h-1.5 rounded-full" style={{ backgroundColor: '#C8102E', boxShadow: '0 0 4px rgba(200,16,46,0.3)' }}></span>
