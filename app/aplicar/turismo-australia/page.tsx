...
import { submitAustraliaApplication } from '././_actions/submit-application'
...
export default function TurismoAustraliaApplication() {
  ...
  const handlePassportUpload = async (file: File) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setPassportStatus('error')
        setPassportError(result.error || 'No pudimos leer el pasaporte. Puedes continuar manualmente.')
      }
    } catch (e) {
      setPassportStatus('error')
      ...
      <div className="space-y-6 max-w-3xl mx-auto">
        ...
        {passportStatus === 'error' && (
          <div className="bg-[#FEF2F2] border border-[#DC2626] p-4 rounded-xl">
            <p className="text-[#DC2626] font-medium mb-4 text-center">{passportError || 'No pudimos leer el pasaporte. Puedes continuar manualmente.'}</p>
        ...
        {passportData && (
          <PassportConfirmModal
            ...
            onConfirm={(confirmed) => {
              ...
              if (confirmed.surname) setValue('step2.family_name' as any, confirmed.surname)
              if (confirmed.given_names) setValue('step2.given_names' as any, confirmed.given_names)
              ...
              if (confirmed.issuing_country) {
                ...
                setValue('step2.country_of_passport' as any, code)
              }
              if (confirmed.nationality) {
                const code = countryNameToCode[confirmed.nationality] || confirmed.nationality
                setValue('step2.nationality_passport_holder' as any, code)
              }
              ...
              if (confirmed.place_of_birth) {
                ...
                if (cleanCity) setValue('step3.birth_town_city' as any, cleanCity)
                if (detectedCountry) setValue('step3.country_of_birth' as any, detectedCountry)
              }
        ...
        <>
          {passportWasScanned && (
            <div className="mb-4 px-4 py-3 rounded-xl text-sm font-medium"
              ...
              ✓ Datos extraídos de tu pasaporte — revisa que todo esté correcto
            </div>
          )}
          <Step1 />
        </>
  ...
  const getStepSubtitle = () => {
    switch (currentStep) {
      ...
      case 10: return 'Declaraciones de salud'
