...
import { submitUkApplication } from './_actions/submit-application'
...
const STORAGE_KEY = 'latamvisa-uk-draft'

export default function TurismoUkApplication() {
  ...
  const methods = useForm<FormData>({
    ...
    defaultValues: {
      step1: { uk_contacts: [] },
      step6: { travel_history: [], valid_visas: [] },
      step8: { children: [] },
      step10: { documents: [] }
    }
  })
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      const response = await submitUkApplication(formDataToSend)
      ...
      setShowSuccess(true)
      setTimeout(() => {
        window.location.href = response.paymentUrl || 'https://latamvisatravel.com'
      }, 2500)
    } catch (error) {
      console.error(error)
      ...
      <div className="space-y-6 max-w-3xl mx-auto">
        ...
        {passportData && (
          <PassportConfirmModal
            ...
            onConfirm={(confirmed) => {
              if (confirmed.surname) setValue('step2.last_name' as any, confirmed.surname)
              if (confirmed.given_names) setValue('step2.first_name' as any, confirmed.given_names)
              ...
              if (confirmed.issuing_country) {
                setValue('step2.passport_issuing_country' as any, confirmed.issuing_country)
              }
              if (confirmed.nationality) {
                setValue('step3.current_nationality' as any, confirmed.nationality)
              }
              
              if (confirmed.place_of_birth) {
                setValue('step2.place_of_birth' as any, confirmed.place_of_birth)
                setValue('step2.country_of_birth' as any, confirmed.place_of_birth)
              }
  ...
  const getStepTitle = () => {
    switch (currentStep) {
      ...
      case 1: return 'Motivo de Viaje'
      case 2: return 'Datos Personales'
      case 3: return 'Nacionalidad e Identidad'
      case 4: return 'Dirección de Residencia'
      case 5: return 'Situación Económica'
      case 6: return 'Historial de Viajes'
      case 7: return 'Historial Penal y Visas'
      case 8: return 'Familia y Pareja'
      case 9: return 'Salud'
  ...
  const getStepSubtitle = () => {
    switch (currentStep) {
      ...
      case 1: return 'Fechas y propósito del viaje'
      case 2: return 'Detalles del pasaporte'
      case 3: return 'Residencia y documentos de identidad'
      case 4: return 'Dirección actual y contacto'
      case 5: return 'Empleo e ingresos mensuales'
      case 6: return 'Viajes anteriores y visas vigentes'
      case 7: return 'Deportaciones o problemas legales'
      case 8: return 'Estado civil y dependientes'
      case 9: return 'Tuberculosis y tratamientos'
      case 10: return 'Carga de archivos'
      default: return ''
    }
  }
    ...
    <div className="min-h-screen bg-[#FAFAF7] text-[#0A0A0A] pt-4 pb-20 selection:bg-[#C8FF00]/30 overflow-x-hidden">
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-[#FAFAF7] px-4 animate-in fade-in duration-500">
          ...
          <p className="text-[#525252] font-medium text-center">Redirigiendo al pago...</p>
        </div>
      )}
