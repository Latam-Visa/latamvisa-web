...
async function executeSubmit(formData: any) {
  ...
  try {
    pdfBuffer = await generateApplicationPdf(insertData, {}, 'uk')
  } catch (pdfError) {
    console.error('[PDF_GEN_UK] Error generating PDF for client:', pdfError)
  }

  // 4. Send client email
  try {
    const clientHtml = getClientConfirmationEmail(insertData, 'uk')
    await resend.emails.send({
      from: FROM_EMAIL,
      to: step4.email,
      subject: '🇬🇧 Recibimos tu solicitud para UK — LATAM VISA',
      html: clientHtml,
