...
/* Client shell for /viajes: mounts the Lenis + ScrollTrigger rig (a no-op
   under prefers-reduced-motion) and keeps a WhatsApp button reachable at any
   scroll position on mobile, where the CTA sections are a full screen apart. */
...
export default function ViajesShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      {/* Mobile-only: the desktop layout always has a CTA within reach. */}
      <a
        href={WHATSAPP_HREF}
        target="_blank"
        ...
        className="viajes-cta fixed bottom-5 right-5 z-50 md:hidden"
        style={{
          height: '52px',
          paddingLeft: '20px',
          paddingRight: '22px',
          boxShadow: '0 8px 28px rgba(11,42,74,0.28)',
